            ...
            (() => {
              ...
              return top ? (
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <div>Open Rate: {top.metrics?.sends ? Math.round((100 * (top.metrics?.opens ?? 0)) / top.metrics.sends) : 0}%</div>
                  <div>CTR: {top.metrics?.sends ? Math.round((100 * (top.metrics?.clicks ?? 0)) / top.metrics.sends) : 0}%</div>
                  <div>Conversion: {top.metrics?.sends ? Math.round((100 * (top.metrics?.conversions ?? 0)) / top.metrics.sends) : 0}%</div>
                </div>
              ) : (
                <div className="mt-4 text-sm text-[var(--muted-foreground)]">No data.</div>
              );
            })()
          )}
        </div>
